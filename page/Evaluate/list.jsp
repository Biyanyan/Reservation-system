    ...
    layui.use(['form','table'],function (){
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: 'orderid',  title: '订单编号',templet:'<div>{{d.order.id}}</div>'},
                // {field: 'username', title: '用户名',templet:'<div>{{d.user.username1}}</div>'},
                // {field: 'workername',  title: '护工姓名',templet:'<div>{{d.careworker.workername}}</div>'},
        ...
        form.on('submit(search-btn)', function (data) {
            ...
            table.reload('currentTableId', {
                url:"${basePath}evaluate/query?orderid="+
                    data.field.orderid,
                contentType:'application/json',
                // where: data.field
            }, 'data');
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {
            if (obj.event === 'add') {   // 监听添加操作
                ...
                var index = layer.open({
                    ...
                    shadeClose: false,
                    area: ['50%', '50%'],
                    content: 'evaluate/detail/'+data[0].id,
                    end:function(){
                        table.reload('currentTableId');
                    }
                });
