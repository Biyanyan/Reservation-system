    ...
    <div class="layuimini-main width_60">
        <form class="layui-form">
                    ...
                    <select name="userid" lay-verify="required">
                        ...
                        <c:forEach items="${users}" var="user">
                            <option value="${user.id}"
                                    <c:if test="${order.userid == user.id}">selected</c:if>
                ...
                <div class="layui-input-block">
                    <select name="workerid" lay-verify="required">
                        ...
                        <c:forEach items="${careworkers}" var="careworker">
                            <option value="${careworker.id}"
                                    <c:if test="${order.workerid == careworker.id}">selected</c:if>
                            >${careworker.workername}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="createtime" value="${order.createtime}" lay-verify="required" class="layui-input">
                </div>
            </div>
...
<script>
    layui.use(['form','jquery'], function () {
        ...
        form.on('submit(save)', function (data) {
            ...
            $.ajax({
                url:"${basePath}order/update?id="+
                    data.field.id+"&username="+
                    data.field.username+"&createtime="+
